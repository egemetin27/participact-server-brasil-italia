...
public abstract class Pipeline {
	...
	public static enum Type {
		 ...
		 * @return The Type represented by <code>value</code>. If the conversion fails,
		 *         returns DUMMY.
		 */
		...
		public static boolean hasPipelineType(List<ActionSensing> list, int enumType) {
			try {
				if (list != null) {
					for (ActionSensing sensor : list) {
						if (sensor != null && sensor.getInput_type() == enumType) {
							return true;
						}
					}
				}
