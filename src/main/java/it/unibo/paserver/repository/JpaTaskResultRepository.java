...
public class JpaTaskResultRepository implements TaskResultRepository {
	...
	public TaskResult addData(long taskId, long userId, Data data) {
		String hql = String.format("from TaskResult t where t.taskReport.task.id = :taskId and t.taskReport.user.id = :userId");
		TypedQuery<TaskResult> query = entityManager.createQuery(hql, TaskResult.class);
