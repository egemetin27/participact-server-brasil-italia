...
public class ResultDataController {
	...
	private static final Logger logger = LoggerFactory.getLogger(ResultDataController.class);
	...
	public @ResponseBody ResponseMessage submitDataAccelerometer(HttpServletRequest request, Principal principal) {
		...
		if (user != null) {
			try {
				...
				response.setKey(key);
				DataAccelerometerProtoList list = DataAccelerometerProtoList.parseFrom(request.getInputStream());
				
				//System.out.println(request.getInputStream().toString());
				
				List<DataAccelerometerProto> dataList = list.getListList();
				logger.info("Received {} accelerometer data items from user {} (KEY: {})", list.getListCount(), user.getOfficialEmail(), key);
				...
				for (DataAccelerometerProto dataAccelerometerProto : dataList) {
					...
					data.setSampleTimestamp(new DateTime(dataAccelerometerProto.getSampleTime()));
				...
				logger.error("Error while receiving data in submitDataAccelerometer", e);
				response.setResultCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
			}
		} else {
	...
	public @ResponseBody ResponseMessage submitDataAccelerometerClassifier(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataAccelerometerClassifierProtoList list = DataAccelerometerClassifierProtoList.parseFrom(request.getInputStream());
				List<DataAccelerometerClassifierProto> dataList = list.getListList();

				logger.info("Received {} accelerometerClassifier data items from user {} (KEY: {})",list.getListCount(), user.getOfficialEmail(), key);
				DateTime now = new DateTime();
				List<Data> resultData = new ArrayList<Data>();
				for (DataAccelerometerClassifierProto netData : dataList) {
					DataAccelerometerClassifier data = new DataAccelerometerClassifier();
					data.setSampleTimestamp(new DateTime(netData.getSampleTime()));
				...
				logger.error("Error while receiving data in submitDataAccelerometerClassifier", e);
				response.setResultCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
			}
		} else {
			response.setResultCode(HttpStatus.UNAUTHORIZED.value());
	...
	public @ResponseBody ResponseMessage submitDataDR(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataDRProtoList list = DataDRProtoList.parseFrom(request.getInputStream());
				List<DataDRProto> dataList = list.getListList();

				logger.info("Received {} dr data items from user {} (KEY: {})", list.getListCount(),
						user.getOfficialEmail(), key);
				DateTime now = new DateTime();
				List<Data> resultData = new ArrayList<Data>();
				...
				logger.error("Error while receiving data in submitDataDR", e);
				response.setResultCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
			}
		} else {
			response.setResultCode(HttpStatus.UNAUTHORIZED.value());
	...
	public @ResponseBody ResponseMessage submitDataActivityRecognitionCompare(HttpServletRequest request,
			Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataActivityRecognitionCompareProtoList list = DataActivityRecognitionCompareProtoList.parseFrom(request.getInputStream());
				List<DataActivityRecognitionCompareProto> dataList = list.getListList();

				logger.info("Received {} activityRecognitionCompare data items from user {} (KEY: {})",list.getListCount(), user.getOfficialEmail(), key);
				DateTime now = new DateTime();
				List<Data> resultData = new ArrayList<Data>();
				for (DataActivityRecognitionCompareProto netData : dataList) {
					...
					data.setGoogleArTimestamp(new DateTime(netData.getGoogleArTimestamp()));
					data.setGoogleArValue(netData.getGoogleArValue());
					data.setMostArTimestamp(new DateTime(netData.getMostArTimestamp()));
	...
	public @ResponseBody ResponseMessage submitDataAppOnScreen(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataAppOnScreenProtoList list = DataAppOnScreenProtoList.parseFrom(request.getInputStream());
				List<DataAppOnScreenProto> dataList = list.getListList();

				DateTime now = new DateTime();
				List<Data> resultData = new ArrayList<Data>();
				logger.info("Received {} AppOnScreen items from user {} (KEY: {})", list.getListCount(), user.getOfficialEmail(), key);
				...
				logger.error("Error while receiving data in submitDataAppOnScreen", e);
				response.setResultCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
			}
		} else {
			response.setResultCode(HttpStatus.UNAUTHORIZED.value());
	...
	public @ResponseBody ResponseMessage submitDataAppsNetTraffic(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataNetTrafficProtoList list = DataNetTrafficProtoList.parseFrom(request.getInputStream());
				List<DataNetTrafficProto> dataList = list.getListList();

				DateTime now = new DateTime();
				List<Data> resultData = new ArrayList<Data>();
				logger.info("Received {} DataNetTraffic (for apps) items from user {} (KEY: {})", list.getListCount(),
						user.getOfficialEmail(), key);
	...
	public @ResponseBody ResponseMessage submitDataBattery(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataBatteryProtoList list = DataBatteryProtoList.parseFrom(request.getInputStream());
				List<DataBatteryProto> dataList = list.getListList();

				DateTime now = new DateTime();
				logger.info("Received {} battery data items from user {} (KEY: {})", list.getListCount(),
						user.getOfficialEmail(), key);
				List<Data> resultData = new ArrayList<Data>();
				...
				logger.error("Error while receiving data in submitDataBattery", e);
				response.setResultCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
			}
		} else {
			response.setResultCode(HttpStatus.UNAUTHORIZED.value());
	...
	public @ResponseBody ResponseMessage submitDataBluetooth(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataBluetoothProtoList list = DataBluetoothProtoList.parseFrom(request.getInputStream());
				List<DataBluetoothProto> dataList = list.getListList();

				DateTime now = new DateTime();
				logger.info("Received {} bluetooth data items from user {} (KEY: {})", list.getListCount(),
						user.getOfficialEmail(), key);
				List<Data> resultData = new ArrayList<Data>();
				...
				logger.error("Error while receiving data in submitDataBluetooth", e);
				response.setResultCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
			}
		} else {
			response.setResultCode(HttpStatus.UNAUTHORIZED.value());
	...
	public @ResponseBody ResponseMessage submitDataConnectionType(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataConnectionTypeProtoList list = DataConnectionTypeProtoList.parseFrom(request.getInputStream());
				List<DataConnectionTypeProto> dataList = list.getListList();

				DateTime now = new DateTime();
				List<Data> resultData = new ArrayList<Data>();
				logger.info("Received {} DataConnectionType items from user {} (KEY: {})", list.getListCount(),
						user.getOfficialEmail(), key);
	...
	public @ResponseBody ResponseMessage submitDataCell(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataCellProtoList list = DataCellProtoList.parseFrom(request.getInputStream());
				List<DataCellProto> dataList = list.getListList();

				DateTime now = new DateTime();
				List<Data> resultData = new ArrayList<Data>();
				logger.info("Received {} cell data items from user {} (KEY: {})", list.getListCount(),
						user.getOfficialEmail(), key);
				for (DataCellProto netData : dataList) {
					...
					data.setBaseStationLatitude(netData.getBaseStationLatitude());
					data.setBaseStationLongitude(netData.getBaseStationLongitude());
	...
	public @ResponseBody ResponseMessage submitDataDeviceNetTraffic(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				...
				DateTime now = new DateTime();
				List<Data> resultData = new ArrayList<Data>();
				logger.info("Received {} DataNetTraffic (for device) items from user {} (KEY: {})", list.getListCount(),
						user.getOfficialEmail(), key);
				for (DataNetTrafficProto netData : dataList) {
	...
	public @ResponseBody ResponseMessage submitDataGoogleActivityRecognition(HttpServletRequest request,
			Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				...
				List<DataGoogleActivityRecognitionProto> dataList = list.getListList();
				logger.info("Received {} GoogleActivityRecognition data items from user {} (KEY: {})",
				...
				logger.error("Error while receiving data in submitDataGoogleActivityRecognition", e);
				response.setResultCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
			}
		} else {
			response.setResultCode(HttpStatus.UNAUTHORIZED.value());
	...
	public @ResponseBody ResponseMessage submitDataGyroscope(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataGyroscopeProtoList list = DataGyroscopeProtoList.parseFrom(request.getInputStream());
				List<DataGyroscopeProto> dataList = list.getListList();
				logger.info("Received {} gyroscope data items from user {} (KEY: {})", list.getListCount(),
						user.getOfficialEmail(), key);
				...
				logger.error("Error while receiving data in submitDataGyroscope", e);
				response.setResultCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
			}
		} else {
			response.setResultCode(HttpStatus.UNAUTHORIZED.value());
	...
	public @ResponseBody ResponseMessage submitDataInstalledApps(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataInstalledAppsProtoList list = DataInstalledAppsProtoList.parseFrom(request.getInputStream());
				List<DataInstalledAppsProto> dataList = list.getListList();

				DateTime now = new DateTime();
				List<Data> resultData = new ArrayList<Data>();
				logger.info("Received {} dataInstalledApps items from user {} (KEY: {})", list.getListCount(),
						user.getOfficialEmail(), key);
				for (DataInstalledAppsProto netData : dataList) {
					...
					data.setRequestedPermissions(netData.getRequestedPermissions());
				...
				logger.error("Error while receiving data in submitDataInstalledApps", e);
				response.setResultCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
			}
		} else {
			response.setResultCode(HttpStatus.UNAUTHORIZED.value());
	...
	public @ResponseBody ResponseMessage submitDataLight(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataLightProtoList list = DataLightProtoList.parseFrom(request.getInputStream());
				List<DataLightProto> dataList = list.getListList();

				logger.info("Received {} light data items from user {} (KEY: {})", list.getListCount(),
						user.getOfficialEmail(), key);
				DateTime now = new DateTime();
				List<Data> resultData = new ArrayList<Data>();
	...
	public @ResponseBody ResponseMessage submitDataLocation(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataLocationProtoList list = DataLocationProtoList.parseFrom(request.getInputStream());
				List<DataLocationProto> dataList = list.getListList();

				logger.info("Received {} dataLocation items from user {} (KEY: {})", list.getListCount(),
						user.getOfficialEmail(), key);
				DateTime now = new DateTime();
				List<Data> resultData = new ArrayList<Data>();
				...
				logger.error("Error while receiving data in submitDataLocation", e);
				response.setResultCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
			}
		} else {
			response.setResultCode(HttpStatus.UNAUTHORIZED.value());
	...
	public @ResponseBody ResponseMessage submitDataMagneticField(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataMagneticFieldProtoList list = DataMagneticFieldProtoList.parseFrom(request.getInputStream());
				List<DataMagneticFieldProto> dataList = list.getListList();

				logger.info("Received {} dataMagneticField items from user {} (KEY: {})", list.getListCount(),
						user.getOfficialEmail(), key);
				DateTime now = new DateTime();
				List<Data> resultData = new ArrayList<Data>();
				...
				logger.error("Error while receiving data in submitDataMagneticField", e);
				response.setResultCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
			}
		} else {
			response.setResultCode(HttpStatus.UNAUTHORIZED.value());
	...
	public @ResponseBody ResponseMessage submitDataPhoneCallDuration(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				...
				logger.info("Received {} dataPhoneCallDuration items from user {} (KEY: {})", list.getListCount(),
						user.getOfficialEmail(), key);
				DateTime now = new DateTime();
				List<Data> resultData = new ArrayList<Data>();
				...
				logger.error("Error while receiving data in submitDataPhoneCallDuration", e);
				response.setResultCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
			}
		} else {
			response.setResultCode(HttpStatus.UNAUTHORIZED.value());
	...
	public @ResponseBody ResponseMessage submitDataPhoneCallEvent(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataPhoneCallEventProtoList list = DataPhoneCallEventProtoList.parseFrom(request.getInputStream());
				List<DataPhoneCallEventProto> dataList = list.getListList();

				logger.info("Received {} dataPhoneCallEvent items from user {} (KEY: {})", list.getListCount(),
						user.getOfficialEmail(), key);
				DateTime now = new DateTime();
				List<Data> resultData = new ArrayList<Data>();
				...
				logger.error("Error while receiving data in submitDataPhoneCallEvent", e);
				response.setResultCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
			}
		} else {
			response.setResultCode(HttpStatus.UNAUTHORIZED.value());
	...
	public @ResponseBody ResponseMessage submitDataSystemStats(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataSystemStatsProtoList list = DataSystemStatsProtoList.parseFrom(request.getInputStream());
				List<DataSystemStatsProto> dataList = list.getListList();

				DateTime now = new DateTime();
				List<Data> resultData = new ArrayList<Data>();
				logger.info("Received {} dataSystemStats items from user {} (KEY: {})", list.getListCount(), user.getOfficialEmail(), key);
				...
				logger.warn("Timed out while reading data from socket"); response.setResultCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
			} catch (Exception e) {
				logger.error("Error while receiving data in submitDataSystemStats", e);
				response.setResultCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
			}
		} else {
			response.setResultCode(HttpStatus.UNAUTHORIZED.value());
	...
	public @ResponseBody ResponseMessage submitDataWifiScan(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataWifiScanProtoList list = DataWifiScanProtoList.parseFrom(request.getInputStream());
				List<DataWifiScanProto> dataList = list.getListList();

				DateTime now = new DateTime();
				List<Data> resultData = new ArrayList<Data>();
				logger.info("Received {} dataWifiScan data items from user {} (KEY: {})", list.getListCount(),user.getOfficialEmail(), key);
				...
				logger.error("Error while receiving data in submitDataWifiScan", e);
				response.setResultCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
			}
		} else {
			response.setResultCode(HttpStatus.UNAUTHORIZED.value());
	...
	public @ResponseBody ResponseMessage submitDataPhoto(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataPhotoProtoList photoProtoList = DataPhotoProtoList.parseFrom(request.getInputStream());

				logger.info("Received {} dataPhoto data items from user {} (KEY: {})", photoProtoList.getListCount(),user.getOfficialEmail(), key);

				for (DataPhotoProto photoProto : photoProtoList.getListList()) {
					try {
						DateTime now = new DateTime();
						Task task = taskService.findById(photoProto.getTaskId());
						Action action = actionService.findById(photoProto.getActionId());
						...
						if (task == null || action == null) {
							...
							logger.warn(
									...
									photoProto.getTaskId(), photoProto.getActionId(), response);
							return response;
						}

						DataPhoto data = new DataPhoto();
						data.setSampleTimestamp(new DateTime(photoProto.getSampleTime()));
						data.setDataReceivedTimestamp(now);
						...
						logger.info("Received photo {} from {} ({}x{}px), {} bytes", filename, user.getOfficialEmail(), data.getWidth(), data.getHeight(), data.getFile().getContent().length);
						data = dataService.merge(data);

						taskResultService.addData(photoProto.getTaskId(), user.getId(), data);
					} catch (NoResultException e) {
						response.setResultCode(ResponseMessage.DATA_NOT_REQUIRED);
						logger.warn("Received photo for task {}, action {}, which are missing. Refusing the upload and returning {}.",photoProto.getTaskId(), photoProto.getActionId(), response);
						return response;
					}
				}
	...
	public @ResponseBody ResponseMessage submitDataQuestion(HttpServletRequest request, Principal principal) {
		ResponseMessage response = new ResponseMessage();
		User user = userService.getUser(principal.getName());
		if (user != null) {
			try {
				String key = request.getHeader(HEADER_KEY);
				response.setKey(key);
				DataQuestionnaireFlatProtoList list = DataQuestionnaireFlatProtoList.parseFrom(request.getInputStream());
				List<DataQuestionnaireFlatProto> dataList = list.getListList();

				DateTime now = new DateTime();
				logger.info("Received {} dataQuestionnaire data items from user {} (KEY: {})", list.getListCount(),
						user.getOfficialEmail(), key);

				Map<Long, List<Data>> taskIdToDataToAdd = new TreeMap<Long, List<Data>>();

				for (DataQuestionnaireFlatProto networkAnswer : dataList) {
					ActionQuestionaire questionnaire = (ActionQuestionaire) actionService.findById(networkAnswer.getActionId());
					if (networkAnswer.getType() == 0) {
						// answer to open question
						DataQuestionaireOpenAnswer data = new DataQuestionaireOpenAnswer();
						data.setSampleTimestamp(new DateTime(networkAnswer.getSampleTime()));
						data.setDataReceivedTimestamp(now);

						data.setOpenAnswerValue(networkAnswer.getOpenAnswerValue());
						Question q = getQuestionById(questionnaire, networkAnswer.getQuestionId());
						data.setQuestion(q);
						data.setUser(user);

						Data savedData = dataService.merge(data);
						addData(taskIdToDataToAdd, networkAnswer.getTaskId(), savedData);
						
						// Pre consolidando
						try {
							QuestionnaireResponseBuilder qrb = new QuestionnaireResponseBuilder();
							QuestionnaireResponse qr = qrb
									.setAll(networkAnswer.getTaskId(), networkAnswer.getActionId(), networkAnswer.getQuestionId(), user.getId(), networkAnswer.getOpenAnswerValue(), 0L, true, false)
									.build(true);
						...
						Question q = getQuestionById(questionnaire, networkAnswer.getQuestionId());
						ClosedAnswer ca = getClosedAnswerById(q, networkAnswer.getAnswerId());
						data.setClosedAnswer(ca);
						data.setClosedAnswerValue(networkAnswer.getClosedAnswerValue());
						data.setUser(user);
						...
						try {
							QuestionnaireResponseBuilder qrb = new QuestionnaireResponseBuilder();
							QuestionnaireResponse qr = qrb.setAll(networkAnswer.getTaskId(), networkAnswer.getActionId(), networkAnswer.getQuestionId(), user.getId(), ca.getAnswerDescription(), networkAnswer.getAnswerId(), networkAnswer.getClosedAnswerValue(), true).build(true);
							questionnaireResponseService.saveOrUpdate(qr);
						} catch (Exception e) {
							e.printStackTrace(System.out);
						}						
	...
	public static void addData(Map<Long, List<Data>> taskIdToDataToAdd, Long taskId, Data data) {
	...
	public Question getQuestionById(ActionQuestionaire questionnaire, long id) {
	...
	public ClosedAnswer getClosedAnswerById(Question question, long id) {
