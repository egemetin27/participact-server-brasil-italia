...
public class TaskServiceImpl implements TaskService {
	...
	public Task assignTaskToUsers(Task task, Collection<String> users) {
		...
		logger.debug("Saved task id {} description \"{}\"", t.getId(), t.getDescription());
		return t;
	}

	@Override
	public List<Task> getAvailableTasksByUser(long userId, TaskState taskCurrentState, DateTime dateTime) {
		return taskRepository.getAvailableTasksByUser(userId, taskCurrentState, dateTime);
	}
	...
	public Task addUsersToTask(long taskId, Collection<String> users) {
		...
		if (task == null) {
			...
			throw new IllegalArgumentException("Task " + taskId + " not found, unable to add users to it");
		}

		if (task.getDeadline().isBefore(new DateTime())) {
			logger.error("Trying adding users to task {} which is already expired", taskId);
			throw new IllegalArgumentException("Task " + taskId + "already expired");
		}
		for (String user : users) {
			User u = userRepository.findByEmail(user);
			if (u == null) {
				logger.error("Cannot add unknown user {} to task {}", user, task.getId());
				throw new IllegalArgumentException("Unknown user " + user);
			}

			try {
				TaskReport taskReport = taskReportRepository.findByUserAndTask(u.getId(), taskId);
				if (taskReport != null) {
					logger.error("Trying to assing task {} to user {} that is already assigned to it", taskId, user);
			...
			logger.debug("Added user {} to task {}", u.getOfficialEmail(), task.getId());
		}
	...
	@Override
	public List<Task> getTaskbyOwner(long userId, TaskValutation currentTaskValutation) {
	...
	@Override
	public List<Task> fetchAllByRemoved(PageRequest pagerequest, DateTime updateDate) {
		return taskRepository.fetchAllByRemoved(pagerequest, updateDate);
	}

	@Override
	public Task findByQrCode(String token) {
		return taskRepository.findByQrCode(token);
	}
}