...
@Controller
public class CkanComcapController extends ApplicationController {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/protected/ckan-comcap/index", method = RequestMethod.GET)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public ModelAndView index(ModelAndView modelAndView) {
		modelAndView.setViewName("/protected/ckan-comcap/index");
	 ...
	 * @return
	 */
	@RequestMapping(value = "/protected/ckan-comcap/download", method = RequestMethod.POST)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public @ResponseBody ResponseJson download(@RequestBody String json, HttpServletRequest request) {
		...
		try {
			...
			try {
				String cmd = String.format(Config.PYTHON_SCRIPT_CKAN_EXPORT, start, deadline, search, parentId);
			...
			e.printStackTrace(System.out);
			System.out.println("CkanComcap Download " + e.getMessage());
		}
		// return
		return response;
	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/protected/ckan-comcap/search/{count}/{offset}", method = RequestMethod.POST)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public @ResponseBody ResponseJson search(@RequestBody String json, @PathVariable int count, @PathVariable int offset) {
		...
		try {
			...
			// Query
			List<Object[]> items = ckanComcapService.search(params, "", true, PaginationUtil.pagerequest(offset, count));
			...
			e.printStackTrace(System.out);
			System.out.println("CkanComcap search " + e.getMessage());
		}
		// return
		return response;
	}
