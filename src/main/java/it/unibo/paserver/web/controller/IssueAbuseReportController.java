...
import it.unibo.paserver.service.IssueAbuseService;
 ...
 */
@Controller
public class IssueAbuseReportController {

	@Autowired
	private IssueAbuseService abuseService;
	@Autowired
	 ...
	 * @return
	 */
	@RequestMapping(value = "/protected/abuse-report/index", method = RequestMethod.GET)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public ModelAndView index(ModelAndView modelAndView) {
		modelAndView.setViewName("/protected/abuse-report/index");
	 ...
	 * @return
	 */
	@RequestMapping(value = "/protected/abuse-report/form", method = RequestMethod.GET)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public ModelAndView form(ModelAndView modelAndView) {
		modelAndView.setViewName("/protected/abuse-report/form");
		modelAndView.addObject("breadcrumb", modelAndView.getViewName());
		modelAndView.addObject("controller", this.getClass().getSimpleName());
		return modelAndView;
	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/protected/abuse-report/edit/{id}", method = RequestMethod.GET)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	 ...
	 */
	@RequestMapping(value = "/protected/abuse-report/edit/{id}/find", method = RequestMethod.GET)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public @ResponseBody ResponseJson find(@PathVariable("id") long id) throws JsonProcessingException {
		// Response
		ResponseJson response = new ResponseJson();
		// Search
		IssueAbuse ia = abuseService.findById(id);
		if (ia != null) {
	 ...
	 * @throws JsonProcessingException
	 */
	@RequestMapping(value = "/protected/abuse-report/removed/{id}", method = RequestMethod.GET)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public @ResponseBody ResponseJson removed(@PathVariable("id") long id) throws JsonProcessingException {
		// Removed
		boolean removed = abuseService.removed(id);
		// Response
		ResponseJson response = new ResponseJson();
	 ...
	 * @return
	 */
	@RequestMapping(value = { "/protected/abuse-report/save/", "/protected/abuse-report/edit/save/" }, method = RequestMethod.POST)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public @ResponseBody ResponseJson save(@RequestBody String json) {
		...
		try {
			if (!Validator.isValidNumeric(id)) {
			...
			} else if (!Validator.isValidStringLength(comment, 1, 1000)) {
				...
				// New or Update
				IssueAbuse ia = abuseService.findById(uuid);
				if (ia != null) {// Edicao
					ia.setComment(comment);
					if (abuseService.saveOrUpdate(ia) != null) {
						response.setStatus(true);
						response.setOutcome(uuid);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/protected/abuse-report/search/{count}/{offset}", method = RequestMethod.POST)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public @ResponseBody ResponseJson search(@RequestBody String json, @PathVariable int count, @PathVariable int offset) {
		...
		try {
			...
			if(issueId>0) {
				...
				// Search
				List<Object[]> items = abuseService.search(params, PaginationUtil.pagerequest(offset, count));
				response.setItems(items);
				if (items.size() > 0) {
					response.setTotal(abuseService.searchTotal(params));
				}
			}
		} catch (Exception e) {
			e.printStackTrace(System.out);
			System.out.println("AbuseReport search " + e.getMessage());
		}
		// return
		return response;
	}
