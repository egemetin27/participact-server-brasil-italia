...
import it.unibo.paserver.domain.support.AbuseTypeBuilder;
import it.unibo.paserver.service.AbuseTypeService;

/**
 * Controller dos tipos de abusos/problemas
 * 
 * @author Claudio
 */
@SuppressWarnings("Duplicates")
@Controller
public class IssueAbuseTypeController {
	@Autowired
	 ...
	 * @return
	 */
	@RequestMapping(value = "/protected/abuse-type/index", method = RequestMethod.GET)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public ModelAndView index(ModelAndView modelAndView) {
		modelAndView.setViewName("/protected/abuse-type/index");
	 ...
	 * @return
	 */
	@RequestMapping(value = "/protected/abuse-type/form", method = RequestMethod.GET)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public ModelAndView form(ModelAndView modelAndView) {
		modelAndView.setViewName("/protected/abuse-type/form");
		modelAndView.addObject("breadcrumb", modelAndView.getViewName());
		modelAndView.addObject("controller", this.getClass().getSimpleName());
		return modelAndView;
	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/protected/abuse-type/edit/{id}", method = RequestMethod.GET)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	 ...
	 */
	@RequestMapping(value = "/protected/abuse-type/edit/{id}/find", method = RequestMethod.GET)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public @ResponseBody ResponseJson find(@PathVariable("id") long id) throws JsonProcessingException {
		// Response
		ResponseJson response = new ResponseJson();
		// Search
		IssueAbuseType u = abuseTypeService.findById(id);
		if (u != null) {
	 ...
	 * @throws JsonProcessingException
	 */
	@RequestMapping(value = "/protected/abuse-type/removed/{id}", method = RequestMethod.GET)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public @ResponseBody ResponseJson removed(@PathVariable("id") long id) throws JsonProcessingException {
		// Removed
		boolean removed = abuseTypeService.removed(id);
		// Response
		ResponseJson response = new ResponseJson();
	 ...
	 * @return
	 */
	@RequestMapping(value = { "/protected/abuse-type/save/", "/protected/abuse-type/edit/save/" }, method = RequestMethod.POST)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public @ResponseBody ResponseJson save(@RequestBody String json) {
		...
		try {
			if (!Validator.isValidNumeric(id)) {
				...
				throw new Exception(messageSource.getMessage("protected.abuseType.name", null, LocaleContextHolder.getLocale()) + '.' + messageSource.getMessage("error.field.exceeded", null, LocaleContextHolder.getLocale()));
			} else {
				// id
				long uuid = Long.parseLong(id);
				// New or Update
				AbuseTypeBuilder ab = new AbuseTypeBuilder();
				if (uuid > 0) {// Edicao
					ab.setId(uuid);
				}
				ab.setAll(uuid, name, description, false);
				// Save
				IssueAbuseType a = ab.build(true);
				IssueAbuseType rs = abuseTypeService.saveOrUpdate(a);
				if (rs != null) {
					response.setStatus(true);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/protected/abuse-type/search/{count}/{offset}", method = RequestMethod.POST)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public @ResponseBody ResponseJson search(@RequestBody String json, @PathVariable int count, @PathVariable int offset) {
		...
		try {
			...
			// Search
			List<Object[]> items = abuseTypeService.search(params, PaginationUtil.pagerequest(offset, count));
			response.setItems(items);
			if (items.size() > 0) {
				response.setTotal(abuseTypeService.searchTotal(params));
			}
		} catch (Exception e) {
			e.printStackTrace(System.out);
			System.out.println("AbuseType search " + e.getMessage());
		}
		// return
		return response;
	}
