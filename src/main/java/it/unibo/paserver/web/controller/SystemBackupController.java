...
import it.unibo.paserver.domain.support.SystemBackupBuilder;
import it.unibo.paserver.service.SystemBackupService;
...
public class SystemBackupController {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/protected/system-backup/index", method = RequestMethod.GET)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public ModelAndView index(ModelAndView modelAndView) {
		modelAndView.setViewName("/protected/system-backup/index");
	 ...
	 * @return
	 */
	@RequestMapping(value = "/protected/system-backup/form", method = RequestMethod.GET)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public ModelAndView form(ModelAndView modelAndView) {
		modelAndView.setViewName("/protected/system-backup/form");
		modelAndView.addObject("breadcrumb", modelAndView.getViewName());
		modelAndView.addObject("controller", this.getClass().getSimpleName());
		return modelAndView;
	}
	 ...
	 * @return
	 */
	@RequestMapping(value = { "/protected/system-backup/save/", "/protected/system-backup/edit/save/" }, method = RequestMethod.POST)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public @ResponseBody ResponseJson save(@RequestBody String json) {
		...
		String hostname = r.getAsString("hostname");
		Long port = r.getAsLong("port");
		String username = r.getAsString("username");
		String password = r.getAsString("password");
		// Validacao
		try {
			if (Validator.isEmptyString(name)) {
				throw new Exception(messageSource.getMessage("name.title", null, LocaleContextHolder.getLocale())
						+ '.'
						+ messageSource.getMessage("error.field.required", null, LocaleContextHolder.getLocale()));

			} else if (Validator.isEmptyString(hostname)) {
				throw new Exception(messageSource.getMessage("hostname.title", null, LocaleContextHolder.getLocale())
						+ '.'
						+ messageSource.getMessage("error.field.required", null, LocaleContextHolder.getLocale()));
				...
				// Build
				SystemBackupBuilder sp = new SystemBackupBuilder();
				// New or Update
				sp.setId(id);
				// Setters
				sp.setAll(id, name, hostname, port, username, password);
				// Save
				SystemBackup p = sp.build(true);
				SystemBackup rs = systemBackupService.saveOrUpdate(p);
				if (rs != null) {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/protected/system-backup/edit/{id}", method = RequestMethod.GET)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	 ...
	 */
	@RequestMapping(value = {"/protected/system-backup/edit/{id}/find","/protected/system-backup/{id}/find"}, method = RequestMethod.GET)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public @ResponseBody ResponseJson find(@PathVariable("id") int id) throws JsonProcessingException {
		// Response
		ResponseJson response = new ResponseJson();
		// Search
		SystemBackup p = systemBackupService.findById(id);
		if (p != null) {
			p.setPassword(null);
			response.setStatus(true);
			response.setItem(p);
	 ...
	 * @throws JsonProcessingException
	 */
	@RequestMapping(value = "/protected/system-backup/removed/{id}", method = RequestMethod.GET)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public @ResponseBody ResponseJson removed(@PathVariable("id") long id) throws JsonProcessingException {
		// Removed
		boolean removed = systemBackupService.delete(id);
		// Response
		ResponseJson response = new ResponseJson();
	 ...
	 * @return
	 */
	@RequestMapping(value = "/protected/system-backup/search/{count}/{offset}", method = RequestMethod.POST)
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public @ResponseBody ResponseJson search(@RequestBody String json, @PathVariable int count,
		...
		// Request
		ReceiveJson r = new ReceiveJson(json);
		List<Object[]> items = systemBackupService.search(PaginationUtil.pagerequest(offset, count));
		if (items.size() > 0) {
			// Translate
			...
			// Total
			response.setTotal(systemBackupService.searchTotal());
		}
		// Return
