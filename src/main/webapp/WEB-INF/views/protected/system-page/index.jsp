...
<!-- BEGIN PAGE-->
<div class="row" ng-controller="SystemPageCtrl" ng-init="initSystemPage();" ng-cloak>
<div class="col-md-12">
		<div class="portlet">
		<!-- BREADCRUMB -->
		<jsp:include page="/WEB-INF/views/protected/system-page/breadcrumb.jsp" />
		<!-- BEGIN TABLE -->
		<div class="portlet-body" ng-show="system_pages.length > '0'">
			<div class="table-scrollable">
				<table class="table table-striped table-bordered table-hover table-checkable" id="<%UUID.randomUUID().toString();%>">
					...
					<tbody>
						<tr ng-repeat="item in system_pages" id="system_pages-tr-{{item[0]}}">
							...
							<td ng-click="editSystemPage(item[0]);" class="qz-pointer">{{item[1] | stripslashes}}							
							</td>
							<td ng-click="editSystemPage(item[0]);" class="qz-pointer">{{item[2].millis | date:'dd/MM/yyyy HH:mm:ss'}}</td>
							<td ng-click="editSystemPage(item[0]);" class="qz-pointer">{{item[3].millis | date:'dd/MM/yyyy HH:mm:ss'}}</td>
							<td>{{item[6]}}<p><small><a href="<c:url value="/pages/"/>{{item[7]}}" target="_blank"><spring:message code="openpage.title" text="Open Page" /></a></small></p></td>
						</tr>
					</tbody>
				</table>
			</div>
		...
		<!-- END TABLE -->
		<!-- BEGIN DISPLAY -->
		<div class="portlet-body" ng-show="system_pages.length == '0'" id="no-data-to-display">
